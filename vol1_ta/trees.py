...
def recursive_search(linkedlist, data):
    ...
    return search(linkedlist.head, data)
    ...
    def insert(self, data):
        ...
        def _step(current, prev, data):
            ...
            if data == current.value:
                raise ValueError("Duplicate nodes not allowed!")
            elif data < current.value:
                return _step(current.left, current, data)
            else:
                return _step(current.right, current, data)
         
        parent = _step(self.root, None, data)
        new.prev = parent
